...
export function getByQuery({ find = {}, or = [{}], sort = { position: 1 }, offset = 0, limit = 10 }) {
  return Promise.all([
...
export function getById(id: any): any {
  return Model.findOne({ _id: id }).lean()
    .then(assertFound(`Article (id ${id}) was not found`));
}
...
export function update(id: any, data: any) {
  return Model.findOneAndUpdate({ _id: id }, { $set: data })
    .then(assertFound(`Could not update article (id ${id})`));
}

export function destroy(id: any) {
  return Model.findOneAndRemove({ _id: id })
    .then(assertFound(`Could not destroy article (id ${id})`));
}
